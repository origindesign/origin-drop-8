...
docker-compose run -w /var/www/html/web php drush pm-enable config_direct_save simple_block responsive_image blazy blazy_ui datetime_range ctools metatag metatag_open_graph token components admin_toolbar admin_toolbar_tools advagg advagg_bundler devel entity_usage features features_ui linkit paragraphs paragraphs_library smart_trim pathauto field_group address media media_library twig_field_value twig_tweak hsts editor_advanced_link link_attributes menu_block redirect redirect_404 redis role_delegation tinypng view_unpublished origin_extend xmlsitemap --yes --strict=0
...
docker-compose run -w /var/www/html/web php drush pm-enable core --yes --strict=0
...
docker-compose run -w /var/www/html/web php drush features-import page article core --yes --strict=0
