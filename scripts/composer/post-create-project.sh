...
docker-compose run -w /var/www/html/web php drush pm-enable config_direct_save simple_block responsive_image blazy blazy_ui datetime_range ctools metatag token components admin_toolbar admin_toolbar_tools advagg devel features features_ui linkit paragraphs smart_trim pathauto field_group address inline_entity_form entity_browser entity_embed entity_browser_entity_form media_entity_browser twig_field_value hsts editor_advanced_link editor_file menu_block redirect redis role_delegation video_embed_field view_unpublished origin_extend --yes --strict=0
